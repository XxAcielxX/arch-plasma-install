...
- Replace `linux` with _linux-hardened_, _linux-lts_ or _linux-zen_ to install the kernel of your choice.
...
```
sudo pacman -S plasma konsole dolphin ark kwrite kcalc spectacle krunner5 partitionmanager packagekit-qt5
```
...
| krunner5         | KDE Quick drop-down desktop search. |
...
Read _[here](#install-oh-my-zsh)_ for customisation & theming for Zsh. Read below how to change your SHELL.
...
We will be using **_Yay_** to install the below two packages as one of them is only available from AUR.
...
Where, _k_ indicates to keep "num" of each package in the cache.
...
Now open your browser and point to it `your-machine-ip:9000` and login with **_root_** to get full access.
